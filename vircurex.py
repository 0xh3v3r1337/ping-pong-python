...
class Vircurex:
    ...
    def simpleRequest(command, **params):
        ...
        url = "{}/api/{}.json?{}".format(Vircurex.domain, command, urllib.parse.urlencode(params)) #url
    ...
    def secureRequest(user, secret, command, tokenparams, **params):
        ...
        txid = hashlib.sha256(txid_bytes).hexdigest()   #unique trasmission ID using random hash
        #token computation
        ...
        url = "{}/api/{}.json?{}".format(Vircurex.domain, command, urllib.parse.urlencode(reqp + params))  #url
